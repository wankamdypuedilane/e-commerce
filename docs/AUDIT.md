...
> **Note d'annotation — 23/09/2026, après le Sprint 11.**
>
> Les sections 1 à 5 restent inchangées. La colonne de statut de la section 6 est désormais datée du **23/09/2026** : chaque ligne a été revérifiée dans le code à cette date. Une ligne dont le statut a changé depuis le 20/09 commence par la mention **« 23/09/2026 »**, avec l'issue ou le commit correspondant ; les autres conservent leur texte du 20/09, numéros de ligne mis à jour lorsqu'ils avaient bougé.
>
> Deux précisions :
>
> - Le workflow `.github/workflows/deploy.yml` cité par les constats a été renommé `ci.yml` au commit `1d37f77`, qui a aussi supprimé le job `deploy`. Les références à `deploy.yml` des colonnes d'origine décrivent l'ancien fichier, consultable dans l'historique.
> - Les identifiants de commit des sections 1 à 5 (`3cf1240`, `65a84de`, `e75041d`…) sont antérieurs à la réécriture de l'historique de l'issue #13 : aucun n'appartient plus à la branche `main`.
>
> Bilan au 23/09/2026 : **15 dettes résolues sur 40**, 1 partiellement résolue (4.7), 3 sans objet, 21 toujours ouvertes. Le Sprint 11 et les jours qui l'ont suivi ont traité les premières dettes de criticité 2 portant sur le code applicatif : 2.1, 2.3 et 2.4.

> **Note d'annotation — 24/09/2026.** La colonne de statut reste datée du 23/09/2026. Trois lignes ont été mises à jour le 24/09, avec la mention **« 24/09/2026 »** : 3.3 (résolue par `330c331`), 4.3 (numéro de ligne) et 4.7 (ajout de Bandit à la CI). Bilan au 24/09/2026 : **16 dettes résolues sur 40**, 1 partiellement résolue (4.7), 3 sans objet, 20 toujours ouvertes.

> **Note d'annotation — 25/09/2026.** Une ligne mise à jour, avec la mention **« 25/09/2026 »** : 2.8, dont le statut décrivait des secrets non chiffrés dans le dépôt comme dans le cluster. Le bilan est inchangé.
...
| 1.3 | Fichiers du projet appartenant à `root`, service tournant en `ubuntu` | `bootstrap.sh` (aucun `chown` global) | Écriture impossible sur SQLite, `media/` et sur le dépôt lors du `git pull` | **Résolue** — #9. `COPY --chown=django:django . .` puis `USER django` dans le `Dockerfile`. Le processus et les fichiers ont le même propriétaire, fixé dans l'image. **23/09/2026** — #71, `6bf7a60` : `/app/media`, absent de l'image, était créé par `root` au montage du volume Compose, et tout téléversement d'image échouait. Le Dockerfile le crée désormais avec le propriétaire `django`. **23/09/2026** — #84, `80278b6` : la résolution de #9 laissait le code modifiable par le processus applicatif, puisque `--chown=django:django` donnait à `django` tous les fichiers copiés. Le code est désormais copié en tant que `root`, sans `--chown`, et `staticfiles/` n'est plus confié à `django`. Vérifié dans une image construite depuis le dépôt : tout `/app` appartient à `root`, sauf `/app/media`, seul emplacement inscriptible par `django` ; l'écriture dans `/app`, `shop/views.py` et `staticfiles/` est refusée. Le processus et les fichiers n'ont donc plus le même propriétaire, et c'est voulu : `django` lit le code sans pouvoir le modifier. Vérifié par `shop/test_image.py`. |
...
| # | Dette | Emplacement | Effet observable | Statut au 23/09/2026 |
|---|---|---|---|---|
| 2.1 | Aucune vérification de propriétaire sur `confirmation`, `payment_success`, `payment_cancel` | `views.py:238,265,291` | Lecture des commandes d'autrui ; `/confirmation` expose la dernière commande du site ; annulation d'une commande tierce en écriture | **23/09/2026 — Résolue** — #70, `dd2e594`. Les trois vues portent `@login_required` et filtrent par `user=request.user` : `views.py:241` (dernière commande de l'utilisateur, non plus du site), `views.py:243` et `views.py:275` (`get_object_or_404(..., user=request.user)`), `views.py:297` (`filter(id=order_id, user=request.user)`). La commande d'autrui répond 404. 8 tests dans `shop/test_acces_commandes.py`. |
...
| 2.3 | Webhook Stripe totalement non testé | `shop/tests.py` | Le seul code qui touche au stock en production n'a aucun filet | **23/09/2026 — Résolue** — #32, `6f177e9`. 8 tests dans `shop/test_webhook.py`, avec une vraie signature Stripe : secret absent, signature invalide, décrément de stock, événement reçu deux fois, email envoyé une seule fois, paiement non confirmé, session expirée, panne pendant le traitement. Ce dernier test a fait passer la réponse en cas d'erreur de 200 à 500. **Réserve :** deux appels simultanés ne sont pas testés (issue #67). |
| 2.4 | `services.py`, `backends.py`, `forms.py` sans aucun test | `shop/tests.py` | Stripe, TVA, emails et connexion par email non couverts | **23/09/2026 — Résolue** — #33, `94a0c70`, `e06abc2`, `6c67a6a`. `shop/test_tva.py` (10 tests), `shop/test_stripe_session.py` (6 tests) et `shop/test_auth.py` (13 tests, backend et formulaires). **Réserve :** 4 des 8 fonctions de `services.py` sont testées directement ; `send_order_confirmation_email` et `sync_commande_payment_from_stripe` ne le sont qu'à travers les tests du webhook. |
| 2.5 | Scripts de sauvegarde pointant sur un chemin inexistant | `scripts/*.sh:4,8` + `docs/ops-backup.md` | Le cron proposé échoue silencieusement : aucune sauvegarde | **Ouverte, contexte déplacé.** `docs/ops-backup.md` est marqué obsolète en tête de fichier et `scripts/` est devenu une archive (#29), mais **aucune sauvegarde n'existe** pour le volume `postgres_data` ni pour le PVC `donnees-postgres-0`. **23/09/2026** — toujours ouverte, suivie par l'issue #83. |
...
| 2.7 | Unicité d'email applicative sans contrainte en base | `forms.py:23-27`, `backends.py:20-23` | Doublons possibles ; le backend peut alors connecter au mauvais compte | **Ouverte.** Aucune migration n'ajoute de contrainte ; `backends.py` conserve la branche `MultipleObjectsReturned`. **23/09/2026** — précision de `e06abc2` : le backend sélectionne le compte le plus ancien et vérifie **son** mot de passe. Le second titulaire est bloqué pour la connexion par email ; il n'est pas connecté au compte d'un autre. Comportement fixé par un test de `shop/test_auth.py`, sans correction. |
| 2.8 | Secrets exposés via `user_data` et `terraform.tfstate` local | `main.tf:88-98`, `versions.tf` | Clés Stripe/Brevo/Django lisibles par l'API EC2 et stockées en clair sur le poste | **Sans objet** — compte AWS fermé, `infra/terraform/` archivé, aucun `apply` possible. Le sujet se déplace : les secrets vivent désormais dans un Secret Kubernetes **encodé en base64, non chiffré** (voir `k8s/02-secrets.example.yaml` et l'[ADR-002](adr/002-kubernetes.md)). **25/09/2026** — toujours sans objet pour AWS. Côté Kubernetes, les secrets sont désormais versionnés chiffrés par SOPS avec une clé age (`k8s/02-secrets.sops.yaml`, [ADR-004](adr/004-gestion-des-secrets.md)) : ils ne sont plus lisibles dans le dépôt sans la clé privée, qui reste hors du dépôt. Une fois appliqués, ils redeviennent un Secret encodé en base64, non chiffré dans le cluster ; le chiffrement au repos n'est pas activé (issue #39). |
...
| 2.10 | `check --deploy` exécuté après le redémarrage, et absent de la CI | `deploy.yml:100` | Une configuration non conforme est mise en ligne avant d'être détectée | **Résolue** — #23. `manage.py check --deploy --fail-level ERROR` est une étape du job `ci` (`deploy.yml:35-41`), exécutée avant tout déploiement. L'occurrence tardive subsiste dans le job `deploy`, lui-même neutralisé. **23/09/2026** — `1d37f77` : l'étape figure désormais dans `ci.yml:50-54`, exécutée dans l'image contre PostgreSQL. Le job `deploy` et son occurrence tardive sont supprimés. |
...
| 3.2 | Déconnexion acceptée en GET | `views.py:412`, `urls.py:54` | Déconnexion déclenchable depuis un site tiers | **Ouverte.** La vue `deconnexion` (`views.py:417`) n'a aucun décorateur de méthode ; la route est déclarée `urls.py:56`. |
| 3.3 | `mark_safe` sur des titres de produits dans l'admin | `admin.py:33` | Injection HTML/JS stockée, visible par les autres administrateurs | **24/09/2026 — Résolue** — `330c331` (Refs #36). Détectée par Bandit (B308, B703). `panier_lisible` construit son HTML avec `format_html_join`, qui échappe chaque valeur insérée ; `mark_safe` n'est plus importé ni appelé nulle part dans `shop/`. Deux tests dans `shop/test_admin.py` : un titre contenant une balise `<script>` est échappé, et la mise en forme est conservée. Vérifié dans les deux sens : Bandit rend le code 0 sur le code actuel et le code 1 sur `admin.py` d'avant la correction. |
| 3.4 | Énumération de comptes par timing | `backends.py:19-24` | Distinction entre email inscrit et inconnu | **23/09/2026 — Résolue** — `e06abc2` (Refs #33). Pour un identifiant inconnu, `backends.py:18-24` hache un mot de passe jetable (`user_model().set_password(password)`) avant de retourner `None`, comme le `ModelBackend` de Django. Le test vérifie l'appel au hachage, pas une durée. |
...
| # | Dette | Emplacement | Statut au 23/09/2026 |
|---|---|---|---|
...
| 4.3 | `inlines = []` écrasé trois lignes plus bas | `admin.py:23` et `47` | **Ouverte.** Inchangé. **24/09/2026** — toujours ouverte ; depuis `330c331`, l'écrasement se trouve à `admin.py:52`. |
...
| 4.6 | Aucun `CSRF_TRUSTED_ORIGINS` malgré le proxy inverse | `settings.py` | **Résolue** — #26. `CSRF_TRUSTED_ORIGINS` construit depuis `DJANGO_CSRF_TRUSTED_ORIGINS` (`settings.py:210-214`). **Réserve :** la liste est vide par défaut et n'est renseignée ni dans `k8s/01-configmap.yaml`, ni sur le cluster. **23/09/2026** — la réserve ne vaut plus pour le dépôt : `1be67ce` a ajouté `DJANGO_CSRF_TRUSTED_ORIGINS: "https://dilane-shop.local"` à `k8s/01-configmap.yaml`. La valeur par défaut reste vide ; l'état du cluster n'a pas été revérifié. |
| 4.7 | Aucune dépendance de développement (ni linter, ni `coverage`) | `requirements.txt` | **23/09/2026 — Partiellement résolue** — #34, `215bac9`. `requirements-dev.txt` inclut `requirements.txt` et ajoute `coverage`, installé seulement dans l'étape `test` du Dockerfile ; la couverture est mesurée en CI avec un seuil en cliquet (`.coveragerc`). **Reste ouvert :** aucun linter ni analyseur statique (issue #36). **24/09/2026** — une analyse statique de sécurité, Bandit, est ajoutée à la CI (issue #36). Bandit n'est pas une dépendance de développement : la CI l'installe dans un conteneur jetable. Reste ouvert : aucun linter. |
...
| 4.12 | Port SSH configurable côté CI mais figé à 22 dans le security group | `deploy.yml:71` vs `main.tf:46-52` | **Sans objet** — le job `deploy` est neutralisé, l'instance EC2 supprimée et le security group n'existe plus. **23/09/2026** — le job `deploy` est supprimé (`1d37f77`) ; le dépôt ne déclare plus aucun secret Actions (`gh secret list` vide). |
